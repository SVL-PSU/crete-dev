        ...
        bool &branch_taken, bool &explored_node) const
{
    ...
    assert(current_tt_node->m_tb_count == tb_index && "[Trace Tag] Assumption broken\n");

    branch_taken = current_tt_node->m_br_taken;
}

// Get the trace-tag for the test case being generated by negating the 'tt_tag_index'th br
        ...
        crete::creteTraceTag_ty &tt_tag_for_tc) const
{
    ...
    if(tt_tag_index > m_trace_tag_explored.size())
    {
        ...
        tt_tag_for_tc.insert(tt_tag_for_tc.end(), m_trace_tag_new.begin(),
                m_trace_tag_new.begin() + (tt_tag_index - m_trace_tag_explored.size()));
    } else {
        tt_tag_for_tc.insert(tt_tag_for_tc.end(), m_trace_tag_explored.begin(),
                m_trace_tag_explored.begin() + tt_tag_index);
    }

    // negate the m_br_taken for the last br as the tc was generated by negating the taken branch;
    tt_tag_for_tc.back().m_br_taken = !tt_tag_for_tc.back().m_br_taken;
}

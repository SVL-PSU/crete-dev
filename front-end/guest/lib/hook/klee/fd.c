...
exe_sym_env_t __exe_env = {
  {{ 0, eOpen | eReadable, 0, 0}, // STDIN_FILENO
   { 1, eOpen | eWriteable, 0, 0}, // STDOUT_FILENO
   { 2, eOpen | eWriteable, 0, 0}}, // STDERR_FILENO
  022,
  0,
  0
};
...
static exe_disk_file_t *__get_sym_file(const char *pathname) {
       ...
       ++i)
  {
        ...
        if (df->path != NULL && strcmp(pathname, df->path) == 0)
        {
            if (df->stat->st_ino == 0)
                return NULL;
          return df;
...
int __fd_open(const char *pathname, int flags, mode_t mode) {
  ...
  if (df) {    
    ...
    // TODO [CRETE]: ignoring permissions for now
...
ssize_t read(int fd, void *buf, size_t count) {
  ...
  if (!f->dfile) {
    ...
    crete_symfile_memcpy(buf, f->dfile->contents + f->off, count);
    f->off += count;
...
ssize_t write(int fd, const void *buf, size_t count) {
  ...
  if (!f->dfile) {
    ...
    if (actual_count)
      crete_symfile_memcpy(f->dfile->contents + f->off, buf, actual_count);
...
int rmdir(const char *pathname) {
  ...
  return syscall(__NR_rmdir, __concretize_string(pathname));
}
...
int unlink(const char *pathname) {
  ...
  return syscall(__NR_unlink, __concretize_string(pathname));
}
...
int unlinkat(int dirfd, const char *pathname, int flags) {
  ...
  return syscall(__NR_unlinkat, dirfd, __concretize_string(pathname), flags);
}
...
int chroot(const char *path) {
  ...
  assert(0 && "ignoring (ENOENT)");
  errno = ENOENT;
  return -1;
}

void klee_warning(const char *name) {
...
int crete_next_available_fd(void)
{
    int fd;
...
/// Returns an existing fd based on the system fd; else, -1.
int crete_existing_fd(int sys_fd)
{
    int fd;
...
{
    assert(0 && "[CRETE] pipe() unsupported!");
}

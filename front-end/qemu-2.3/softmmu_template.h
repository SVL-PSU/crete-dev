                            ...
                            uintptr_t retaddr)
{
    ...
    if(flag_rt_dump_enable)
        fprintf(stderr, "%s has been called. tb->pc = %x. Input addr is: %x. mmu_idx is: %d.\n",
                __FUNCTION__, rt_dump_tb->pc, addr, mmu_idx);
...
//            assert(0 && "[Ld Memo Monitor] le IO access");
    ...
    do_unaligned_access:
        ...
        if(flag_rt_dump_enable) {
#if defined(CRETE_DEBUG)
            // FIXME: xxx unaligned access
            fprintf(stderr, "[Ld Memo Monitor] le slow unaligned access: addr = %p, size = %u, res = 0x%lu\n",
                    (void *)(uint64_t)addr, (uint32_t)DATA_SIZE, (uint64_t)res);
#endif // defined(CRETE_DEBUG)
            dump_memo_sync_table_entry(runtime_env, addr, DATA_SIZE, res);
        }
    ...
    res = glue(glue(ld, LSUFFIX), _le_p)((uint8_t *)haddr);
        ...
        if(flag_rt_dump_enable) {
#if defined(CRETE_DEBUG)
            fprintf(stderr, "[Ld Memo Monitor] le: addr = %p, size = %u, res = 0x%lu\n",
                    (void *)(uint64_t)addr, (uint32_t)DATA_SIZE, (uint64_t)res);
#endif // defined(CRETE_DEBUG)
            dump_memo_sync_table_entry(runtime_env, addr, DATA_SIZE, res);
        }
                            ...
                            uintptr_t retaddr)
{
    ...
    if (unlikely(tlb_addr & ~TARGET_PAGE_MASK)) {
        ...
        if(flag_rt_dump_enable) {
#if defined(CRETE_DEBUG) || 1
            fprintf(stderr, "[Ld Memo Monitor] be IO access: addr = %p, size = %u, res = 0x%lu\n",
                    (void *)(uint64_t)addr, (uint32_t)DATA_SIZE, (uint64_t)res);
            assert(0 && "[Ld Memo Monitor] be IO access\n");
#endif // defined(CRETE_DEBUG)
            dump_memo_sync_table_entry(runtime_env, addr, DATA_SIZE, res);
        }
#endif // #if defined(CRETE_ENABLE_MEMORY_MONITOR)
    ...
    do_unaligned_access:
        ...
        if(flag_rt_dump_enable) {
#if defined(CRETE_DEBUG) || 1
            fprintf(stderr, "[Ld Memo Monitor] be slow unaligned access: "
                    "addr = %p, size = %u, res = 0x%lu\n",
                    (void *)(uint64_t)addr, (uint32_t)DATA_SIZE, (uint64_t)res);
            assert(0 && "[Ld Memo Monitor] be slow unaligned access\n");
#endif // defined(CRETE_DEBUG)
            dump_memo_sync_table_entry(runtime_env, addr, DATA_SIZE, res);
        }
#endif // #if defined(CRETE_ENABLE_MEMORY_MONITOR)
    ...
    res = glue(glue(ld, LSUFFIX), _be_p)((uint8_t *)haddr);
        ...
        if(flag_rt_dump_enable) {
#if defined(CRETE_DEBUG) || 1
            fprintf(stderr, "[Ld Memo Monitor] be: "
                    "addr = %p, size = %u, res = 0x%lu\n",
                    (void *)(uint64_t)addr, (uint32_t)DATA_SIZE, (uint64_t)res);

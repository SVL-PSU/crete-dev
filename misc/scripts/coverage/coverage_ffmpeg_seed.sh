...
function main
{
    ...
    foldername="seed-coverage$(date +[%Y-%m-%d_%T])"
    mkdir $foldername
    cd $foldername
    ...
    do
        ...
        $COVCMD --seed-only -g -e $PROG_DIR/$target_prog -c $f/guest/crete-guest-config.xml -t $f/test-case >> crete-coverage-progress.log
    done
